...
/* Wrapped in an IIFE so its top-level names ($, esc, State, init, …) stay
   private and don't collide with app.js (which shares the classic-script
   global scope). Only window.FWMaps is exported. */
(function () {
...
function gotoMatch(e) {
  ...
  e.marker.openPopup();
  $('#map-search-results').classList.remove('show');
}
...
function wireUI() {
  ...
  document.addEventListener('click', e => { if (!e.target.closest('#map-search-wrap')) $('#map-search-results').classList.remove('show'); });
  search.onkeydown = e => {
    const box = $('#map-search-results');
  ...
  // keyboard shortcuts — only while the Maps tab is actually showing, so they
  // don't fire (or steal '/') on the Weapons/Attachments/… tabs.
  document.addEventListener('keydown', e => {
    if (!document.body.classList.contains('maps-active')) return;
...
function wireOfflineSave() {
  ...
  // The service worker is registered once by the host (app.js); here we only
  // wire the offline-save button + its progress messages.
...
/* Public surface for the host (app.js). init() boots the atlas once, lazily,
   the first time the Maps tab is opened; invalidateSize() re-fits Leaflet
   whenever the tab is shown (Leaflet mis-measures while display:none). */
window.FWMaps = {
  init,
